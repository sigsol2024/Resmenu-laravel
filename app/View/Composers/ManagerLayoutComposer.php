...
class ManagerLayoutComposer
{
    ...
    public function compose(View $view): void
    {
        ...
        $showOrders = $restaurantId > 0 && $this->features->foodOrderingUsable($restaurantId);
        $showReservations = $restaurantId > 0 && $this->features->tableReservationsUsable($restaurantId);
        $showPaymentSettings = $restaurantId > 0 && (
            $this->features->planHasFoodOrdering($restaurantId)
            || $this->features->planHasTableReservations($restaurantId)
        );
        ...
        $view->with([
            ...
            'layoutNavItems' => $this->buildNavItems($showOrders, $showReservations, $showPaymentSettings),
    ...
    private function buildNavItems(bool $showOrders, bool $showReservations, bool $showPaymentSettings): array
    {
        $items = [
        ...
        $items = array_merge($items, [
            ['id' => 'sections', 'name' => 'Sections', 'href' => route('manager.sections.index'), 'icon' => 'M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5'],
            ...
            ['id' => 'customization', 'name' => 'Menu Designs', 'href' => route('manager.customization'), 'icon' => 'M9.53 16.122a3 3 0 00-5.78 1.128 2.25 2.25 0 01-2.4 2.245 4.5 4.5 0 008.4-2.245c0-.399-.078-.78-.22-1.128zm0 0a15.998 15.998 0 003.388-1.62m-5.043-.025a15.994 15.994 0 011.622-3.395m3.42 3.42a15.995 15.995 0 004.764-4.648l3.876-5.814a1.151 1.151 0 00-1.597-1.597L14.146 6.32a15.996 15.996 0 00-4.649 4.763m3.42 3.42a6.776 6.776 0 00-3.42-3.42'],
        ...
        if ($showPaymentSettings) {
            $items[] = ['id' => 'payment-settings', 'name' => 'Payment Settings', 'href' => route('manager.billing.payment-settings'), 'icon' => 'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z'];
        }

        $items[] = ['id' => 'settings', 'name' => 'Settings', 'href' => route('manager.settings.edit'), 'icon' => 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065zM15 12a3 3 0 11-6 0 3 3 0 016 0z'];
