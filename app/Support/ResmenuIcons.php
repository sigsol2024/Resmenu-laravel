...
class ResmenuIcons
{
    ...
    public static function icon(string $name, array $options = []): string
    {
        ...
        $classAttr = $class !== ''
            ? ' class="resmenu-icon resmenu-icon--'.e($resolved).' '.e($class).'"'
            : ' class="resmenu-icon resmenu-icon--'.e($resolved).'"';
        $style = isset($options['style']) && $options['style'] !== ''
            ? ' style="'.e((string) $options['style']).'"'
            : '';
        $aria = isset($options['label']) && $options['label'] !== ''
            ? ' role="img" aria-label="'.e((string) $options['label']).'"'
            : ' aria-hidden="true"';
    ...
    public static function passwordToggle(int $size = 20, string $class = ''): string
    {
        $cls = trim('resmenu-password-toggle-icons '.$class);

        return '<span class="'.e($cls).'" data-resmenu-password-icons>'
            .self::icon('eye', ['size' => $size, 'class' => 'resmenu-icon-eye'])
            .self::icon('eye-off', ['size' => $size, 'class' => 'resmenu-icon-eye-off hidden'])
            .'</span>';
    }
