...
<div class="shrink-0">
<?php $t16BrandLogo = (!empty($isTemplatePreview)) ? null : resmenu_logo_url($uploadBaseUrl ?? '', $restaurant['logo'] ?? null); ?>
<?php if ($t16BrandLogo): ?><img src="<?php echo htmlspecialchars($t16BrandLogo); ?>" alt="<?php echo htmlspecialchars($restaurant['name']); ?>" class="<?php echo htmlspecialchars(nmc_img_class('h-11 w-11 md:h-12 md:w-12', $restaurant['logo'], 'logos', 'rounded-xl'), ENT_QUOTES, 'UTF-8'); ?>"/><?php else: ?><div class="flex h-11 w-11 rotate-12 items-center justify-center rounded-xl font-black text-xl brand-gradient md:h-12 md:w-12 md:text-2xl"><?php echo strtoupper(substr($restaurant['name'], 0, 1)); ?></div><?php endif; ?>
</div>
