...
<section id="reservation" class="py-xl scroll-mt-28">
...
<a href="#" class="inline-block bg-primary text-on-primary px-8 py-4 font-label-lg text-label-lg uppercase tracking-widest w-fit opacity-80 cursor-not-allowed">Reserve Now (Demo)</a>
<?php else: ?>
<p class="font-label-md text-label-md uppercase text-on-surface-variant mb-4">Complete your booking below</p>
<?php endif; ?>
</div>
<div class="relative min-h-[420px] lg:min-h-[560px] order-1 lg:order-2 bg-surface-container-high">
<?php if (! empty($isTemplatePreview)): ?>
...
<?php else: ?>
<iframe
    src="<?php echo t6_esc($reservationUrl ?? ''); ?>"
    title="Table reservation for <?php echo t6_esc($restaurant['name'] ?? ''); ?>"
    class="w-full h-full min-h-[420px] lg:min-h-[560px] border-0 absolute inset-0 bg-surface-container-low"
    loading="lazy"
></iframe>
<?php endif; ?>
</div>
