...
$t6Primary = $rfd['primaryColor'] ?? '#f0be78';
...
?>
<?php if ($t6ResSuccess): ?>
<div class="mb-6 p-4 rounded border border-primary/30 bg-primary/10 text-on-surface font-body-sm">
Reservation confirmed! We look forward to seeing you.
</div>
<?php endif; ?>
<?php if (! empty($t6FormErrorList)): ?>
<div class="mb-6 p-4 rounded border border-red-800/40 bg-red-950/30 text-red-200 font-body-sm space-y-1">
...
<div class="res-step-indicator w-7 h-7 md:w-8 md:h-8 rounded-full font-semibold flex items-center justify-center shrink-0 text-on-primary" id="step-ind-1" data-step="1" style="background-color:<?php echo t6_esc($t6Primary); ?>">1</div>
...
<button type="button" id="res-cal-prev" class="p-2 rounded text-on-surface-variant hover:text-primary transition-colors"><span class="material-symbols-outlined">chevron_left</span></button>
<span id="res-cal-month" class="font-label-lg text-label-lg text-on-surface"></span>
<button type="button" id="res-cal-next" class="p-2 rounded text-on-surface-variant hover:text-primary transition-colors"><span class="material-symbols-outlined">chevron_right</span></button>
</div>
...
<span><span class="inline-block w-2.5 h-2.5 rounded-sm bg-primary/60 mr-1"></span>Available</span>
...
<button type="button" id="party-minus" class="t6-res-qty-btn w-9 h-9 flex items-center justify-center rounded-full border border-outline-variant/40 text-on-surface hover:border-primary hover:text-primary transition-colors"><span class="material-symbols-outlined text-lg">remove</span></button>
...
<button type="button" id="party-plus" class="t6-res-qty-btn w-9 h-9 flex items-center justify-center rounded-full border border-outline-variant/40 text-on-surface hover:border-primary hover:text-primary transition-colors"><span class="material-symbols-outlined text-lg">add</span></button>
</div>
</div>
</div>
<div>
...
?>
<button type="button" data-time="<?php echo t6_esc($slot['time'] ?? ''); ?>" class="time-slot t6-time-slot py-2.5 px-1 text-xs sm:text-sm font-label-md rounded border border-outline-variant/40 text-on-surface bg-surface-container-low hover:border-primary transition-all<?php echo $avail ? '' : ' opacity-40 cursor-not-allowed line-through'; ?>"<?php echo $avail ? '' : ' disabled'; ?>><?php echo t6_esc($slot['time'] ?? ''); ?></button>
<?php endforeach; ?>
</div>
...
<input name="guest_name" type="text" placeholder="Full Name" required class="t6-res-input w-full px-4 py-3 bg-surface-container-low border-b border-outline-variant/30 text-on-surface font-body-md outline-none focus:border-primary transition-colors" value="<?php echo t6_esc($t6Old('guest_name', '')); ?>">
<input name="guest_email" type="email" placeholder="Email Address" required class="t6-res-input w-full px-4 py-3 bg-surface-container-low border-b border-outline-variant/30 text-on-surface font-body-md outline-none focus:border-primary transition-colors" value="<?php echo t6_esc($t6Old('guest_email', '')); ?>">
</div>
<input name="guest_phone" type="tel" placeholder="Phone Number" required inputmode="numeric" class="t6-res-input w-full px-4 py-3 bg-surface-container-low border-b border-outline-variant/30 text-on-surface font-body-md outline-none focus:border-primary transition-colors mb-6" value="<?php echo t6_esc($t6Old('guest_phone', '')); ?>">
...
<button type="button" data-occasion="<?php echo t6_esc($occ); ?>" class="occasion-btn t6-occasion-btn px-3 py-2 text-xs font-label-md uppercase rounded-full border border-outline-variant/40 text-on-surface-variant bg-surface-container-low hover:border-primary transition-colors"><?php echo t6_esc(str_replace('_', ' ', $occ)); ?></button>
<?php endforeach; ?>
</div>
<input type="hidden" name="special_occasion" id="special-occasion-input" value="<?php echo t6_esc($t6Old('special_occasion', '')); ?>">
<textarea name="notes" rows="3" placeholder="Dietary requirements or additional notes..." class="t6-res-input w-full px-4 py-3 bg-surface-container-low border-b border-outline-variant/30 text-on-surface font-body-md outline-none focus:border-primary transition-colors resize-none mb-6"><?php echo t6_esc($t6Old('notes', '')); ?></textarea>
<div class="flex justify-between gap-3">
<button type="button" class="res-back-btn t6-res-btn-ghost px-5 py-3 font-label-lg text-label-lg uppercase tracking-widest">Back</button>
<button type="button" class="res-next-btn t6-res-btn-primary px-6 py-3 font-label-lg text-label-lg uppercase tracking-widest">Next</button>
</div>
...
<?php if ($t6Deposit > 0): ?>
<p class="mb-4 text-on-surface-variant font-body-sm">A deposit of <strong class="text-primary">₦<?php echo number_format($t6Deposit, 2); ?></strong> will be required at checkout.</p>
<?php endif; ?>
<div class="flex justify-between gap-3">
<button type="button" class="res-back-btn t6-res-btn-ghost px-5 py-3 font-label-lg text-label-lg uppercase tracking-widest">Back</button>
...
<script src="<?php echo t6_esc($t6SiteBase); ?>/assets/js/reservation-wizard.js"></script>
